...
public class ClientDragonRenderer {
    ...
    public static DragonEntity getOrCreateDragon(final Player player) {
        return ClientDragonRenderer.PLAYER_DRAGON_MAP.computeIfAbsent(player.getId(), key -> {
            DragonEntity newDragon = DSEntities.DRAGON.get().create(player.level(), EntitySpawnReason.COMMAND);
    ...
    public static void clearEntries(final LevelEvent.Unload event) {
        ...
        DragonRenderer.clearRenderStates();
    }

    @SubscribeEvent
    public static void renderAbilityHitbox(final RenderLevelStageEvent.AfterOpaqueBlocks event) {
    ...
    public static void cancelNameplatesFromDummyEntities(RenderNameTagEvent.CanRender renderNameplateEvent) {
        if (renderNameplateEvent.getEntityRenderState().entityType == DSEntities.DRAGON.get()) {
