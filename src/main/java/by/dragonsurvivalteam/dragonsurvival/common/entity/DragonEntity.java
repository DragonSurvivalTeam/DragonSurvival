...
public class DragonEntity extends LivingEntity implements GeoEntity, EntityScale.Override {
    ...
    private PlayState predicate(final AnimationState<DragonEntity> state) {
        ...
        if (player.getAbilities().flying || ServerFlightHandler.isFlying(player)) {
            ...
            state.setAnimation(DragonAnimations.FLY_LAND_END.getAnimation());
        } else if (AttachmentManager.getExistingData(player, DSDataAttachments.CLIMBABLE_DATA).map(data -> data.getClimbingType() != SyncClimbFlag.ClimbingType.NONE).orElse(false) || player.onClimbable()) {
