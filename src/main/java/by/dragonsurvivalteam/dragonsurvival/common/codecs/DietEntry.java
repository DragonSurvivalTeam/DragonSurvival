...
public record DietEntry(List<String> items, FoodProperties properties) {
    ...
    public static Map<ResourceKey<Item>, FoodProperties> map(final List<DietEntry> entries) {
        Map<ResourceKey<Item>, FoodProperties> diet = new HashMap<>();
        entries.forEach(entry -> entry.items().forEach(item -> ResourceLocationWrapper.map(item, BuiltInRegistries.ITEM).forEach(key -> diet.put(key, entry.properties()))));
        return diet;
    }
}
