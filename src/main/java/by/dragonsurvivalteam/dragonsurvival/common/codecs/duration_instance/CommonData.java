...
public record CommonData(ClientEffectProvider.ClientData clientData, Optional<ResourceKey<DragonAbility>> ability, Optional<UUID> source, int appliedAbilityLevel) {
    ...
    public static CommonData from(final ServerPlayer dragon, final DragonAbilityInstance ability, final Optional<ResourceLocation> customIcon) {
        ClientEffectProvider.ClientData clientData = ClientEffectProvider.ClientData.from(dragon, ability, customIcon);
        return new CommonData(clientData, Optional.of(ability.key()), Optional.of(dragon.getUUID()), ability.level());
    }
    ...
    public static CommonData from(final Holder<DragonPenalty> penalty, final Optional<ResourceLocation> customIcon) {
        ...
        return new CommonData(clientData, Optional.empty(), Optional.empty(), 1);
    }
}