...
public class SilentGemsDatapack extends BlockLootSubProvider {
    ...
    protected void generate() {
        DSBlocks.REGISTRY.getEntries().forEach((key) -> {
            if (key.get() instanceof ModCompat compat && ModID.SILENTGEMS.value().equals(compat.getCompatId())) {
    ...
    protected @NotNull Iterable<Block> getKnownBlocks() {
        return DSBlocks.REGISTRY.getEntries().stream().map(DeferredHolder::get)
                .filter(block -> block instanceof ModCompat compat && ModID.SILENTGEMS.value().equals(compat.getCompatId()))
