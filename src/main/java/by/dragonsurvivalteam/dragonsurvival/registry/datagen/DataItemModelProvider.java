            ...
            if (holder.get() instanceof BlockItem blockItem && BLOCK_MODELS_AS_BASIC.stream().noneMatch(blockItem.toString()::contains)) {
                if (blockItem.toString().contains("skeleton")) {
                    ...
                    getBuilder(blockItem.toString())
                            .parent(new ModelFile.UncheckedModelFile(new ResourceLocation(DragonSurvival.MODID, BLOCK_FOLDER + "/" + split[0])))
                            .texture("skeleton_texture", new ResourceLocation(DragonSurvival.MODID, BLOCK_FOLDER + "/" + "skeleton_dragon_" + skin))
                            .transforms().transform(ItemDisplayContext.GUI).rotation(30, 160, 0).scale(0.5f).end();
                } else if (blockItem.toString().contains("vault")) {
                    getBuilder(blockItem.toString()).parent(new ModelFile.UncheckedModelFile(new ResourceLocation(DragonSurvival.MODID, BLOCK_FOLDER + "/" + holder.getId().getPath()) + "_inactive"));
                } else if (blockItem.getBlock() instanceof TreasureBlock) {
                    // Show the 1 layer texture
                    ResourceLocation parent = new ResourceLocation(DragonSurvival.MODID, BLOCK_FOLDER + "/" + holder.getId().getPath() + "2");
                    ...
                    getBuilder(blockItem.toString())
                            .parent(new ModelFile.UncheckedModelFile(new ResourceLocation(DragonSurvival.MODID, BLOCK_FOLDER + "/" + holder.getId().getPath())))
                            .texture("1", new ResourceLocation(DragonSurvival.MODID, BLOCK_FOLDER + "/" + "primordial_anchor_empty"));
                } else {
                    getBuilder(blockItem.toString()).parent(new ModelFile.UncheckedModelFile(new ResourceLocation(DragonSurvival.MODID, BLOCK_FOLDER + "/" + holder.getId().getPath())));
                }
            } else {
                if (holder.get() instanceof SpawnEggItem item) {
                    getBuilder(item.toString()).parent(new ModelFile.UncheckedModelFile(new ResourceLocation("item/template_spawn_egg")));
                    ...
                    getBuilder(itemLoc.toString())
                            ...
                            .texture("layer0", new ResourceLocation(DragonSurvival.MODID, "item/" + itemLoc.getPath()));
                } else {
