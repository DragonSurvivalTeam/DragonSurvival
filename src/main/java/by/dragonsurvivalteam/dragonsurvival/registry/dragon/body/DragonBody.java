...
public record DragonBody(
    ...
    public static final StreamCodec<FriendlyByteBuf, Holder<DragonBody>> STREAM_CODEC = ByteBufCodecs.holderRegistry(REGISTRY);
    ...
    public static String getWingButtonName(final Holder<DragonBody> holder) {
        ...
        return Translation.Type.BODY_WINGS.wrap(holder.unwrapKey().orElseThrow().location());
    }

    public static String getWingButtonDescription(final Holder<DragonBody> holder) {
        //noinspection DataFlowIssue -> key is present
        return Translation.Type.BODY_WINGS_DESCRIPTION.wrap(holder.unwrapKey().orElseThrow().location());
    }
}
