...
public record DietEntryRemover(String key) implements DataMapValueRemover<DragonSpecies, List<DietEntry>> {
    public static final Codec<DietEntryRemover> CODEC = ResourceLocationWrapper.validatedCodec().xmap(DietEntryRemover::new, DietEntryRemover::key);
    ...
    public @NotNull Optional<List<DietEntry>> remove(@NotNull final List<DietEntry> value, @NotNull final Registry<DragonSpecies> registry, @NotNull final Either<TagKey<DragonSpecies>, ResourceKey<DragonSpecies>> source, @NotNull final DragonSpecies species) {
        ...
        newDiet.removeIf(entry -> entry.items().equals(key));
